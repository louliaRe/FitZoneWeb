      ...
      {Array.isArray(gyms) && gyms.length > 0 ? (
        gyms.map((gym) => (
          <Card key={gym.id} shadow="sm" padding="lg" className={classes.card1}>
            ...
            <Group
              position="apart"
              style={{ marginBottom: 5, marginTop: "20px" }}
            >
              <Text className={classes.name}>{gym.name}</Text>
              <Badge color="lime" variant="light">
                {gym.fees.length > 0
                  ? `${gym.fees[0].type} : ${gym.fees[0].fee} - ${gym.fees[1].type} : ${gym.fees[1].fee}`
                  : ""}
              </Badge>
            </Group>
            ...
            <Button
              ...
              fullWidth
              style={{ marginTop: 14 }}
              onClick={() => handleViewBranches(gym.id)}
            >
              view branches
            </Button>
        ...
        <Text color="white">No gyms available</Text>
      )}
    </div>
  );
};
