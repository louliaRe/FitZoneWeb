...
import { TextInput, NumberInput, Checkbox, Select, Container, Button, Group } from "@mantine/core";
import { useAuth } from "../../AuthContext";
import classes from './TrainingPlan.module.css';
import {CreatePlan, getPlan, getExerciseseq } from "../../ApiServices/TrainingScheduleAPI";
...
const TrainingPlan = () => {
    ...
    useEffect(() => {
        const fetchExercises = async () => {
            try {
                const data = await getExerciseseq(authState.accessToken);
                
                const formattedData = data.map(ex => ({
                    label: ex.name,
                    value: ex.exercise_id.toString(),
                }));
        ...
        try {
            ...
            workouts.forEach((workout, workoutIndex) => {
                if (!workout.is_rest && !workout.name) {
                    formErrors[`workoutName_${workoutIndex}`] = 'Workout name is required';
                }
                if (workout.has_cardio && workout.cardio_duration <= 0) {
                    formErrors[`cardioDuration_${workoutIndex}`] = 'Cardio duration must be greater than 0';
                }
                workout.exercises.forEach((exercise, exerciseIndex) => {
                    if (!exercise.exercise) {
                        formErrors[`exercise_${workoutIndex}_${exerciseIndex}`] = 'Exercise is required';
                    }
                    if (exercise.sets <= 0) {
                        formErrors[`sets_${workoutIndex}_${exerciseIndex}`] = 'Sets must be greater than 0';
                    }
                    if (exercise.rest_time_seconds < 0) {
                        formErrors[`restTime_${workoutIndex}_${exerciseIndex}`] = 'Rest time cannot be negative';
                    }
                });
            });
            ...
            if (Object.keys(formErrors).length === 0) {
                ...
                const createPlan = await CreatePlan(authState.accessToken, authState.gym_id ,data);
    ...
    return (
        <Container className={classes.con}>
            ...
            {workouts.map((workout, workoutIndex) => (
                <div key={workoutIndex} className={classes.workout}>
                    <TextInput
                        ...
                        value={workout.name}
                        onChange={(event) => handleWorkoutChange(workoutIndex, 'name', event.currentTarget.value)}
                    ...
                    <Checkbox
                        ...
                        onChange={(event) => handleWorkoutChange(workoutIndex, 'is_rest', event.currentTarget.checked)}
                    />
                    {!workout.is_rest && (
                        <>
                            <Checkbox
                                ...
                                onChange={(event) => handleWorkoutChange(workoutIndex, 'has_cardio', event.currentTarget.checked)}
                            />
                            {workout.has_cardio && (
                                <NumberInput
                                    label="Cardio Duration (minutes)"
                                    value={workout.cardio_duration}
                                    onChange={(value) => handleWorkoutChange(workoutIndex, 'cardio_duration', value)}
                                    required
                                    min={1}
                                    error={errors[`cardioDuration_${workoutIndex}`]}
                                />
                            )}
                            <Button color="lime" onClick={() => handleAddExercise(workoutIndex)}>
                                Add Exercise
                            </Button>
                            {workout.exercises.map((exercise, exerciseIndex) => (
                                <div key={exerciseIndex} className={classes.exercise}>
                                    <Select
                                        ...
                                        onChange={(value) => handleExerciseChange(workoutIndex, exerciseIndex, 'exercise', Number(value))}
                                        required
                                        error={errors[`exercise_${workoutIndex}_${exerciseIndex}`]}
                                    />
                                    <NumberInput
                                        label="Sets"
                                        value={exercise.sets}
                                        onChange={(value) => handleExerciseChange(workoutIndex, exerciseIndex, 'sets', value)}
                                        required
                                        min={1}
                                    ...
                                    {Array.from({ length: exercise.sets }).map((_, setIndex) => (
                                        <NumberInput
                                            ...
                                            onChange={(value) => handleRepsChange(workoutIndex, exerciseIndex, setIndex + 1, value)}
                                            required
                                        />
                                    ))}
                                    <NumberInput
                                        ...
                                        onChange={(value) => handleExerciseChange(workoutIndex, exerciseIndex, 'rest_time_seconds', value)}
                                        required
                                        min={0}
                                        error={errors[`restTime_${workoutIndex}_${exerciseIndex}`]}
                                    />
                                </div>
                            ))}
                        </>
            ...
            <Button color="lime" className={classes.btn} onClick={handleAddTrainingPlan}>
                Submit Plan
            </Button>
        </Container>
    );
