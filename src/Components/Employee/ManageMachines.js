  ...
  const im = 'http://localhost:8000/'
    ...
    useEffect(() => {
      const fetchMachinesInside = async () => {
        try {
          const response = await GetMachinesInside(authState.accessToken, hall.id);
          console.log('API Response:', response);
  
          if (Array.isArray(response) && response.length > 0 && response[0].equipment) {
            const equipmentData = response[0].equipment.map(eq => ({
              ...eq,
    ...
    const handleSaveMachine = async () => {
      try {
        ...
        await AddMachineToCertainHall(authState.accessToken, hall.id, positionData);
      ...
      try {
        ...
        if (newMachine.exercises?.length) {
          newMachine.exercises.forEach(exercise => {
            formData.append('exercises', JSON.stringify({ exercise_id: parseInt(exercise) }));
          });
        }
        ...
        setMachines([...machines, { label: createdMachine.name, value: createdMachine.id.toString() }]);
    ...
    const handleUpdateMachine = async (machine) => {
      try {
        ...
        await UpdateDiagram(authState.accessToken, hall.id, authState.branch_id, updatedMachineInfo);
    ...
    return (
      <Container>
        ...
        <Modal opened={isModalOpen} onClose={() => setIsModalOpen(false)} title={viewMode ? 'Machine Details' : 'Edit Machine Position'} centered>
          ...
          {currentMachine ? (
            <>
              ...
              <NumberInput
                ...
                onChange={(value) => setCurrentMachine(prevState => ({ ...prevState, x_axis: value }))}
              />
              <NumberInput
                label="Position Y"
                name="y_axis"
                value={currentMachine.y_axis}
                onChange={(value) => setCurrentMachine(prevState => ({ ...prevState, y_axis: value }))}
              />
              {error && <div className={classes.error}>{error}</div>}
              <Button color="lime" onClick={()=>handleUpdateMachine(currentMachine)} style={{ marginTop: '10px' }}>
                Update Position
              </Button>
            </>
          ) : (
            <>
              <Select
                ...
                onChange={(value) => setExistingMachinePosition(prevState => ({ ...prevState, machine_id: value }))}
              />
              <NumberInput
                label="Position X"
                name="x"
                value={existingMachinePosition.x}
                onChange={(value) => setExistingMachinePosition(prevState => ({ ...prevState, x: value }))}
              />
              <NumberInput
                label="Position Y"
                name="y"
      ...
      />
        ...
        <Modal opened={addModalOpen} onClose={() => setAddModalOpen(false)} title="Add New Machine" centered>
          ...
          <Select
            ...
            onChange={(value) => handleChange({ target: { name: 'exercises', value } })}
          />
          <MultiSelect
            ...
            onChange={(value) => setNewMachine(prevState => ({ ...prevState, diseases: value }))}
          />
          <FileInput
            type="file"
            name="video_path"
