...
const TrainingPlanDisplay = () => {
  ...
  const { training_plan_data: planData, plan_duration_weeks: duration } =
    trainingPlan;
      ...
      <Container className={classes.container}>
        ...
        {planData.workouts.map((workout, workoutIndex) => (
          <Card
            ...
            p="lg"
            mb="lg"
            className={classes.workoutCard}
          >
            <Group className={classes.group}>
              <Text className={classes.textLarge}>
                {workout.name || `Day ${workout.order}`}
              </Text>
              {workout.is_rest ? (
                <Badge className={`${classes.badge} ${classes.badgeRest}`}>
                  Rest Day
                </Badge>
              ) : (
                <Badge className={`${classes.badge} ${classes.badgeWorkout}`}>
                  Workout
                </Badge>
              )}
            </Group>
            {!workout.is_rest && (
              <>
                <Text className={classes.textMedium} mt="md">
                  Cardio:{" "}
                  {workout.has_cardio
                    ? `Yes, ${workout.cardio_duration} mins`
                    : "No"}
                </Text>
                  ...
                  {workout.exercises.map((exercise, exerciseIndex) => (
                    <List.Item key={exerciseIndex} className={classes.listItem}>
                      <Text className={classes.textMedium}>
                        ...
                        {exercise.exercises_details.exercise_details.name}
                      </Text>
                      <Text className={classes.textSmall}>
                        Sets: {exercise.sets}, Rest Time:{" "}
                        {exercise.rest_time_seconds} seconds
                      </Text>
                      <Text className={classes.textSmall}>
                        Reps:{" "}
                        {Object.entries(exercise.reps)
                          .map(([set, reps]) => `Set ${set}: ${reps}`)
                          .join(", ")}
                      </Text>
                    </List.Item>
                  ))}
