...
import { UpdatePost, deletePost, getPosts , accept } from '../../ApiServices/EmpServices';
...
const ManagePosts = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const postData = await getPosts(authState.accessToken, authState.gym_id);
  ...
  const handleApprove = async (postId) => {
    try {
      ...
      setPosts(posts.map(post => (post.id === postId ? { ...post, is_approved: true } : post)));
  ...
  const handleDeny = async (postId) => {
    try {
      ...
      setPosts(posts.map(post => (post.id === postId ? { ...post, is_approved: false } : post)));
  ...
  const handleDelete = async(postId)=>{
   ...
   try{
      const resdelete= await deletePost(authState.accessToken, postId,authState.gym_id)
  ...
  const handleEdit = async (postId, editedPost) => {
    ...
    try {
      ...
      // }
      const response = await UpdatePost(authState.accessToken, postId, authState.gym_id, editedPost);
      setPosts(posts.map(post => (post.id === postId ? { ...post, ...response } : post)));
    } catch (error) {
